...
class JungleWorldMapTest {
    ...
    void testAddAndRemoveElement() {
        ...
        assertTrue(map.elementsAt(new Vector2d(99, 29)).contains(animal2));
        map.addElement(animal3);
        assertTrue(map.elementsAt(new Vector2d(99, 29)).contains(animal3));
    ...
    void testOnPositionChange() {
        ...
        map.addElement(animal1);
        animal1.move();
        assertNull(map.elementsAt(new Vector2d(99, 29)));
        assertTrue(map.elementsAt(animal1.getPosition()).contains(animal1));
        map.removeElement(animal1);
        map.addElement(animal2);
        animal2.move();
        assertNull(map.elementsAt(new Vector2d(50, 29)));
        assertTrue(map.elementsAt(animal2.getPosition()).contains(animal2));
        map.removeElement(animal2);
        map.addElement(animal3);
        map.addElement(animal4);
        animal3.move();
        animal4.move();
        assertTrue(map.elementsAt(animal3.getPosition()).contains(animal3));
        assertTrue(map.elementsAt(animal4.getPosition()).contains(animal4));
    }
    ...
    void testGetUnoccupiedPosition() {
        JungleWorldMap map = new JungleWorldMap(new Rectangle(new Vector2d(0, 0), new Vector2d(9, 9)));
